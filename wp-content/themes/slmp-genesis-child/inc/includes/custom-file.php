...
class create_file {
	public function __construct() {
		if( class_exists('ACF') ) {
			add_action('acf/save_post', array(&$this, 'create_dynamic_custom_backup'));
			add_action('acf/save_post', array(&$this, 'delete_dynamic_custom_backup'));
	...
	/* Create Dynamic Folder and File backup */
	public function create_dynamic_custom_backup() {
	    if ( function_exists('acf_add_options_page') ) {
	        $time           = time();
	        $new_time       = date("Y-m-d H:i:s",$time);

	        if ( !is_dir( SLMP_THEME_UPLOAD_DIR . '/slmp-dynamic-gen/dynamic-custom-backup/' . $new_time ) ) {
			    mkdir( SLMP_THEME_UPLOAD_DIR . '/slmp-dynamic-gen/dynamic-custom-backup/' . $new_time , 0777, true );
			}

			$css_file       = SLMP_THEME_UPLOAD_DIR . '/slmp-dynamic-gen/dynamic-custom-backup/'. $new_time .'/custom-css.css';
	        $js_file        = SLMP_THEME_UPLOAD_DIR . '/slmp-dynamic-gen/dynamic-custom-backup/'. $new_time .'/custom-js.js';
	        $function_file  = SLMP_THEME_UPLOAD_DIR . '/slmp-dynamic-gen/dynamic-custom-backup/'. $new_time .'/custom-function.php';
	        ...
	        if (!is_file($js_file)) {
	            ...
	            file_put_contents($js_file, $contents);
	            chmod($js_file, 0777);
	        }
	        if (!is_file($function_file)) {
	            $contents = get_field('custom_function', 'option');
	            file_put_contents($function_file, $contents);
	            chmod($function_file, 0777);
	        }
	    }
    }

    /* Delete Dynamic Folder and File Backup Pass 1day */
	public function delete_dynamic_custom_backup() {

	    $backup_files = glob(wp_upload_dir()['basedir'] . '/slmp-dynamic-gen/dynamic-custom-backup/' . '*');
	    $threshold          = (86400);
	    $current_time       = time();
	    ...
	    if ( $backup_files ) {
	        foreach ( $backup_files as $backup ) {
	            $file_d     = filemtime($backup);
	            $difference = $current_time - $file_d;

	            if ( $difference >= $threshold ) {
	                $deleted_folders[] = $backup;
	            }
	        }
	    }

	    if ( $deleted_folders ) {
	        foreach ( $deleted_folders as $file ) {
	...
	public function create_dynamic_shortcode_file() {
		if ( function_exists('acf_add_options_page') ) {
			...
			// Create Dynamic Shortcode Backup
			$time           	= time();
	        $new_time       	= date("Y-m-d H:i:s",$time);
			$shortcode_file     = SLMP_THEME_UPLOAD_DIR . '/slmp-dynamic-gen/dynamic-custom-backup/'. $new_time .'/dynamic-shortcode.php';

			if (!is_file($shortcode_file)) {
	            file_put_contents($shortcode_file, $contents);
	            chmod($shortcode_file, 0777);
	        }
		}
	}
	...
	public function create_dynamic_hook_file() {
		if ( function_exists('acf_add_options_page') ) {
			...
			// Create Dynamic Hook Backup
			$time           = time();
	        $new_time       = date("Y-m-d H:i:s",$time);
			$hook_file      = SLMP_THEME_UPLOAD_DIR . '/slmp-dynamic-gen/dynamic-custom-backup/'. $new_time .'/dynamic-hook.php';

			if (!is_file($hook_file)) {
	            file_put_contents($hook_file, $dynamik_hook_boxes);
	            chmod($hook_file, 0777);
	        }
		}
	}
